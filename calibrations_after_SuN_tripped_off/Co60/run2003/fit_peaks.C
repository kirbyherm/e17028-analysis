...
std::vector<double> double_gaussian_and_linear_background(TH1F* hist, std::vector<double> x_peaks_ordered){
  ...
  hist->Fit(Form("fitting_function_%s",hist->GetName()),"Q 0 B","", // Do not + here because the final fitting function fits the entire spectrum
  ...
  hist->Fit(Form("fitting_function_%s",hist->GetName()),"Q + B","",
  ...
  hist->Fit(Form("gaus_1_component_%s",hist->GetName()),"Q + B");

  TF1* gaus_2_component = new TF1(Form("gaus_2_component_%s",hist->GetName()),"gaus(0)",0,30000);
  gaus_2_component->FixParameter(0, fitting_function->GetParameter(3));
  gaus_2_component->FixParameter(1, fitting_function->GetParameter(4));
  gaus_2_component->FixParameter(2, fitting_function->GetParameter(5));
  gaus_2_component->SetLineColor(3);
  hist->Fit(Form("gaus_2_component_%s",hist->GetName()),"Q + B");
  
  TF1* line_component = new TF1(Form("line_component_%s",hist->GetName()),"pol1(0)",0,30000);
  line_component->FixParameter(0, fitting_function->GetParameter(6));
  line_component->FixParameter(1, fitting_function->GetParameter(7));
  line_component->SetLineColor(5); // Histogram color is blue, so do not choose 4
  hist->Fit(Form("line_component_%s",hist->GetName()),"Q + B");
...
std::vector<double> gaussian_and_linear_background(TH1F* hist, std::vector<double> x_peaks_ordered){
  ...
  for (unsigned int i=0; i<x_peaks_ordered.size(); i++){
    ...
    hist->Fit(fitting_function,"Q +","",
	      fitting_function->GetParameter(1) - 1.5 * fitting_function->GetParameter(2),
	      fitting_function->GetParameter(1) + 1.5 * fitting_function->GetParameter(2));
...
std::vector<double> single_gaussians(TH1F* hist, std::vector<double> x_peaks_ordered){
  ...
  for (unsigned int i=0; i<x_peaks_ordered.size(); i++){
    ...
    hist->Fit(fitting_function,"Q +","",
	      temp_gaus->GetParameter(1) - 1.5 * temp_gaus->GetParameter(2),
	      temp_gaus->GetParameter(1) + 1.5 * temp_gaus->GetParameter(2));
...
void fit_peaks(){
  ...
  for (unsigned int i=0; i<Halves.size(); i++){ // Halves
    for (int j=0; j<4; j++){ // Segments
      ...
      hist->GetXaxis()->SetRangeUser(2000,5000);

      TSpectrum* spc = new TSpectrum(2);
