...
void create_histograms(){
  ...
  chain->Add("/mnt/analysis/e17028/rootfiles/run2006-*.root");
  ...
  //     chain->Project(Form("E%s%d(32000,1,32001)",Halves[i].c_str(),j+1),
  // 		     Form("E%s%d",Halves[i].c_str(),j+1),
  // 		     "E_Si>0 && E_Si<10000", // Gate on the silicon detector
  // 		     "");
  //     fOut->cd();
  //     ((TH1F*) gDirectory->Get(Form("E%s%d",Halves[i].c_str(),j+1)))->Write();
