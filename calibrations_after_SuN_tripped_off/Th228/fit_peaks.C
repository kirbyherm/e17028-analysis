...
std::vector<double> single_gaussians(TH1F* hist, std::vector<double> x_peaks_ordered){
  ...
  for (unsigned int i=0; i<x_peaks_ordered.size(); i++){
    ...
    // Remove unrealistic standard deviations that will mess up some fits
    //std::cout << temp_gaus->GetParameter(2) << std::endl;
    if (temp_gaus->GetParameter(2) < 20 || temp_gaus->GetParameter(2) > 300){
      temp_gaus->SetParameter(2, 30);
    }
...
void fit_peaks(){
  ...
  for (unsigned int i=0; i<Halves.size(); i++){ // Halves
    for (int j=0; j<4; j++){ // Segments
      ...
      // For TSpectrum(10), was finding the 2614 gamma in all segments except EB4. Therefore, break into two conditions.
      TSpectrum* spc;
      std::string hist_name = hist->GetName();
      if (hist_name != "EB4"){
	spc = new TSpectrum(10);
      }
      if (hist_name == "EB4"){
	spc = new TSpectrum(13);
      }
