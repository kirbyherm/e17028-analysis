...
void decays(){
    std::vector<std::string> pixels = {"1","3","5"};
    for (int pix_i=0; pix_i<pixels.size(); pix_i++){
...
//        std::string filepath = "/mnt/analysis/e17028/rootfiles/run" + run_str + "-*.root";
        std::string filepath = "/mnt/analysis/e17028/correlated_rootfiles_NERO/1000ms/true/Run" + run_str + "_true_" + pixels[pix_i] + "_md_0_v61.root";
...
//    save_to_png(c2, "pid_pin2tof");    
    c2->Clear();
    chain->Draw(("cStripHighBack:cStripHighFront>>decays(40,1,41,40,1,41)"),(""),"colz");
    TH2F* pid_final = ((TH2F*) gDirectory->Get("decays"));
//    c2->SetLogz();
//    c2->SetLogy();
...
//    TH1F* pid_final = ((TH1F*) gDirectory->Get("implants"));
    save_to_png(c2, "decays_"+pixels[pix_i]);    
    c2->Clear();
